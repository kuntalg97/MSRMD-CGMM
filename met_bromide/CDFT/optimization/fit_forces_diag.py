...
params['De_cbr_intra']        = 2.7879315074606756
params['De_cbr_inter']        = 2.6214393921070203
params['alpha_cbr_intra']     = 0.9944980802555274
params['alpha_cbr_inter']     = 0.0004908468402119958
params['re_cbr_morse']        = 1.7641423053101213
params['OFFDIAG_G']           = 0.0 
params['V_IJ_CONSTANT']       = 0.0
params['DIAG_VII_CONST']      = 8.5
names = ['global_morse_cut', 'De_cbr_intra', 'De_cbr_inter', 'alpha_cbr_intra', 'alpha_cbr_inter', 're_cbr_morse', 'OFFDIAG_G', 'V_IJ_CONSTANT', 'DIAG_VII_CONST']

bounds = np.array([[0,5.0],[0.0,None],[0.0,None],[0.0,None],[0.0004908468402119958,None],[0,None],[0,None],[None,None],[7.0,10.0]])
...
def write_input(params):
    ...
    for line in fpin:
       ...
       line = re.sub("De_cbr_intra", str(params['De_cbr_intra']), line)
       line = re.sub("De_cbr_inter", str(params['De_cbr_inter']), line)
       line = re.sub("alpha_cbr_intra", str(params['alpha_cbr_intra']), line)
       line = re.sub("alpha_cbr_inter", str(params['alpha_cbr_inter']), line)
       line = re.sub("re_cbr_morse", str(params['re_cbr_morse']), line)
       fpout.write(line)
    ...
    for i in range(len(names)):
       n = names[i]
       if n in ['global_morse_cut', 'De_cbr_intra', 'De_cbr_inter', 'alpha_cbr_intra', 'alpha_cbr_inter', 're_cbr_morse']:
          continue
